...
export default function Tracking() {
  ...
  const { info } = useSelector((state: RootState) => state.addToOrderData);
  const [delivery, setDelivery] = useState<any>(null);
  const id = localStorage.getItem("tracking-id");
  const deliveryData = useSelector((state) =>
    id ? selectDeliveryById(state, id) : null
  );
  ...
  }, [delivery?.address?.lat, delivery?.address?.lng]);

  // Get customer address from finalOrder
  const customerAddress = delivery?.address;
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <Button
            onClick={() => router.push("/")}
            className="mt-4 [box-shadow:var(--shadow-m)] bg-[#FF8080] hover:bg-[#FF8080]/80 text-white"
          >
            Return to Menu
          </Button>
        </div>
  ...
  return (
    <>
      ...
      <div className="min-h-screen">
        {/* Header */}
        <div className="bg-white border-b border-[#f0f0f0] rounded-bl-3xl  px-2 py-3 [box-shadow:var(--shadow-s)] flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Order #{delivery?.orderId} •{info.name}
              </p>
            </div>
          </div>
          <Button
            ...
            className="border border-gray-200 [box-shadow:var(--shadow-m)] bg-[#FF8080] hover:bg-[#FF8080]/80 text-white"
          >
            <RefreshCw
        ...
        <div className="relative h-[65vh] bg-gray-200">
          ...
          <div className="absolute bottom-4 right-4 z-10">
            <Button
              ...
              variant="secondary"
              className={cn(
                "rounded-full w-12 h-12 [box-shadow:var(--shadow-m)] bg-[#FF8080] hover:bg-[#FF8080]/80 text-white",
            ...
            <Card className="[box-shadow:var(--shadow-s)]">
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm mb-4">
                  ...
                  {delivery?.payment?.discount?.amount > 0 ||
                    (delivery?.payment?.discount?.amount > 0 && (
                      <div className="flex justify-between text-xs text-gray-500">
                        <span>
                          Savings with{" "}
                          <span>
                            {delivery?.payment?.discount?.code ||
                              delivery?.payment?.discount?.code}
                          </span>
                        </span>
                        <span className="flex items-center text-green-600 text-sm font-bold  ">
                          - <IndianRupee className="w-3 h-3" />
                          {delivery?.payment?.discount?.amount ||
                            delivery?.payment?.discount?.amount}
                        </span>
                      </div>
                    ))}
